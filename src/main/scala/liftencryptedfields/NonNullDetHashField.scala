...
class NonNullDetHashField[OwnerType <: Record[OwnerType]](rec:OwnerType, override val systemKey:Array[Byte]) 
  extends StringField(rec, 86) with DetHashField[OwnerType] with StringAsBinary[OwnerType] {
  protected def binHashSet(plainText:Array[Byte]) = apply(binHash_?(plainText))
}
